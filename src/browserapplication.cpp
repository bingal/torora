...
void BrowserApplication::postLaunch()
{
    ...
    // through Tor.
    setTor(true);
    BrowserApplication::torManager()->checkTorInstallation(false);
#else
...
{
    {
        ...
        if (settings.value(QLatin1String("restoring"), false).toBool()) {
            QMessageBox::StandardButton result = QMessageBox::question(0, tr("Restore failed"),
                tr("Mgeni crashed while trying to restore this session.  Should I try again?"), QMessageBox::Yes | QMessageBox::No);
...
TorManager *BrowserApplication::torManager()
{
    if (!s_torManager) {
